...
async def pet_command(message: types.Message):
    ...
    await message.reply_to_message.reply(
        f"Пользователь {message.from_user.first_name} поблагодарил пользователя {message.reply_to_message.from_user.first_name}")
...
async def mute_handler(message: types.Message):
    if message.chat.type != "supergroup" or message.chat.type != "group":
        ...
        if message.reply_to_message is not None:
            user = await message.bot.get_chat_member(chat_id=message.chat.id,
                                                     user_id=message.reply_to_message.from_user.id)
            if user.status == "creator" or user.status == "administrator":
                await message.reply(
                    text="Этот пользователь создатель группы или администратор, я не могу его замутить!!!")
            else:
                await message.reply_to_message.reply(
                    text=f"Выдать мут пользователю id: {message.reply_to_message.from_user.id}?",
                    reply_markup=keyboards.mute_keyboard())
        else:
        ...
        if user.status == "administrator" or user.status == "creator":
            await message.bot.restrict_chat_member(
                ...
                permissions=types.ChatPermissions(can_send_messages=True, can_send_games=True,
                                                  ...
                                                  can_invite_users=True, can_add_web_page_previews=True,
